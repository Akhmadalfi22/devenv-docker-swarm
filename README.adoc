...
http://localhost/whoami | link:docker-compose.traefik.yml[] | - Traefik ({traefik-version}) +
- whoami ({traefik-whoami-version}) |

|<<nginx>> | http://localhost:7233 | link:docker-compose.nginx.yml[] | NGINX ({nginx-version}) |
...
|<<postgres>> | http://localhost:5432 | link:docker-compose.postgres.yml[] | Postgres ({postgres-version}) | POSTGRES_REPLICAS

|<<mongo>> | http://localhost:27017 | link:docker-compose.mongo.yml[] | Mongo ({mongo-version}) | MONGO1_REPLICAS +
MONGO2_REPLICAS +
MONGO3_REPLICAS +
MONGO_INIT_REPLICAS

|<<redis>> | http://localhost:6379 | link:docker-compose.redis.yml[] | Redis ({redis-version}) | REDIS_REPLICAS

|<<influxdb>> | http://localhost:8086 | link:docker-compose.influxdb.yml[] | InfluxDB ({influxdb-version}) | INFLUXDB_REPLICAS

|<<neo4j>> | http://localhost:7474 +
http://localhost:7687 | link:docker-compose.neo4j.yml[] | Neo4j ({neo4j-version}) | NEO4J_REPLICAS

|<<keycloak>> | http://localhost/keycloak/ | link:docker-compose.keycloak.yml[] | Keycloak ({keycloak-version}) | KEYCLOAK_REPLICAS

|<<kafka>> | http://localhost:9092 | link:docker-compose.kafka.yml[] | Kafka ({kafka-version}) | KAFKA_REPLICAS

|<<maildev>> | http://localhost:1080 | link:docker-compose.maildev.yml[] | MailDev ({maildev-version}) | MAILDEV_REPLICAS
...
|<<prometheus>> | http://localhost:9090 | link:docker-compose.prometheus.yml[] | Prometheus ({prometheus-version}) | PROMETHEUS_REPLICAS
